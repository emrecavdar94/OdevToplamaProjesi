...
namespace OdevToplamaProjesi.Web.Ayarlar
{
    public class ResimIslem
    {
        public string Ekle(HttpPostedFileBase orjResim)
        {
            ...
            res.Save(HttpContext.Current.Server.MapPath("/Content/Resimler/Ogrenci/" + ad));
            return ad;
        }

        public string Sil(string resimAdi)
        {
            string yol = HttpContext.Current.Server.MapPath("/Content/Resimler/Ogrenci/" + resimAdi);
