...
{
    public class ResimIslemOgretmen
    {
        public string Ekle(HttpPostedFileBase orjResim)
        {
            ...
            res.Save(HttpContext.Current.Server.MapPath("/Content/Resimler/Ogretmen/" + ad));
            return ad;
        }

        public string Sil(string resimAdi)
        {
            string yol = HttpContext.Current.Server.MapPath("/Content/Resimler/Ogretmen/" + resimAdi);
